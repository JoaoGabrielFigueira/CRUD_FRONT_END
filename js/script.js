...
//Falso banco de dados de estilos
var estilos = [];
...
function buscar() {
    ...
    fetch(`http://localhost:3000/bodybuilder/search/${busca}`, {
      method: "GET",
      headers: {
...
function salvarEstilo() {
    let nome = document.getElementById("estilo").value; 
  ...
  function carregarListaEstilos() {
    fetch("http://localhost:3000/style", {
      ...
      .then((response) => response.json())
      .then((data) => {
        estilos = data; //recebe a lista de academias
        atualizarListaEstilos();
      })
      .catch((error) => {
        console.log(error);
